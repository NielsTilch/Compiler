...
public class Parcours {
    ...
    private static int taille_int = 2;
    private static int taille_adr= 2;
    ...
    public static void parcoursTree(Tree tree, TDS tds, FileWriter f, int nbChild) throws IOException {
        ...
        switch (tree.getType()) {
            ...
            case bloodResetLexer.CLASSDEF:
                ...
                //semantique.SemanticAnalyser.analyseClasse(tree, tds, f, hasExtend);
            ...
            case bloodResetLexer.METHODEDEF:
                ...
                for (int i=0; i < tree.getChild(decalage+1).getChildCount(); i++) {              //--> lecture des paramètres
                    parcoursTree(tree.getChild(decalage+1).getChild(i), tdsMeth, f, tree.getChild(decalage+1).getChild(i).getChildCount());
                }
                ...
                if (tree.getChild(decalage+2).getText().equals("TYPERETOUR")) {
                    ...
                    if (tree.getChild(decalage + 2).getChild(0).getText().equals("Integer")) {       //--> rajout automatique du result dans la tds de la meth avec TYPERETOUR
                        tdsMeth.addElement(false, tree.getChild(decalage + 2).getChild(0).getText(),
                                true, "result", -taille_int);
                    } else if (tree.getChild(decalage + 2).getChild(0).getText().equals("String")) {
                        ...
                        tdsMeth.addElement(false, tree.getChild(decalage + 2).getChild(0).getText(),
                                false,
                                "result",
                                -taille_adr); //déplacement d'une taille d'adresse
                    }
            ...
            case bloodResetLexer.FORMALPARAM:   //même traitement pour param, formal param, et declvariable
                ...
                if (tree.getChild(1).getText().equals("Integer")) {
                    tds.addElement(false, tree.getChild(1).getText(),
                            true, tree.getChild(0).getText(), -taille_int);
            ...
            case bloodResetLexer.PARAM:     //même traitement pour param, formal param, et declvariable
                ...
                if (tree.getChild(1).getText().equals("Integer")) {
                    ...
                    tds.addElement(false, tree.getChild(1).getText(),
                            (tree.getChild(1).getText().equals("String") || tree.getChild(1).getText().equals("Integer")),
                            tree.getChild(0).getText(),
                            -taille_adr); //taille d'une adresse
                }
                break;
                ...
                if (tree.getChild(0).getText().equals("FORMALPARAM")){
                    ...
                    if (tree.getChild(0).getChild(1).getText().equals("Integer")) {
                        tds.addElement(false, tree.getChild(0).getChild(1).getText(),
                                true, tree.getChild(0).getChild(0).getText(), taille_int);

                    } else if (tree.getChild(1).getText().equals("String")) {
                        //deplacement String = taille d'une adresse vers le tas
                        //adresse de 64bytes ?
                        tds.addElement(false, tree.getChild(0).getChild(1).getText(),
                                true, tree.getChild(0).getChild(0).getText(),taille_adr);

                    } else {
                        //deplacement = taille classe
                        tds.addElement(false, tree.getChild(0).getChild(1).getText(),
                                false, tree.getChild(0).getChild(0).getText(),taille_adr); //taille d'une adresse
                    }
                    ...
                    if (tree.getChild(1).getText().equals("Integer")) {
                        ...
                        //adresse de 64bytes ?
                        tds.addElement(false, tree.getChild(1).getText(),
                                true, tree.getChild(0).getText(), taille_adr);

                    } else {
                        //deplacement = taille classe
                        tds.addElement(false, tree.getChild(1).getText(),
                                false, tree.getChild(0).getText(), taille_adr);  //taille d'une adresse
                    }
            ...
            case bloodResetLexer.ATTRIBDEF:
                ...
                if (classType.equals("Integer")) {
                    ...
                    tds.addElement(isStaticAttrib, classType, true, name,taille_int);


                } else if (classType.equals("String")) {
                    //deplacement String = taille d'une adresse vers le tas
                    //adresse de 16bytes == 2 octets?
                    tds.addElement(isStaticAttrib, classType, true, name,taille_adr);


                } else {
                    //deplacement = taille classe
                    tds.addElement(isStaticAttrib, classType, false, name,taille_adr); //taille d'une adresse;

                }
                break;

            /* en fait c'est un default
            case bloodReset.bloodResetLexer.BLOCCLASSE :
            ...
            case bloodResetLexer.IF:
                TDS tdsThen = new TDS(tds, "THEN", "then"+TDS.getCountIf());
                tdsThen.addElement(tds);                //on ajoute la ligne correspondant à la tds père dans la tdsThen

                TDS tdsElse = new TDS(tds, "ELSE", "else"+TDS.getCountIf());
            ...
            * */
            ...
            case bloodResetLexer.MESSOUSELECT:
                if (tree.getChild(1).getText().equals("MESSAGE")) {

                    int i = 2;
                    String typeMess = SemanticAnalyser.getTypeExpression(tree.getChild(0), tds, f);


                    typeMess = SemanticAnalyser.getReturnMethodeFromClass(tree, typeMess, tree.getChild(1).getChild(0).getText(), tds, f);
                    if (typeMess != null) {
                        while (i < tree.getChildCount() && tree.getChild(i).getText() == "MESSAGE") {
                            typeMess = SemanticAnalyser.getReturnMethodeFromClass(tree, typeMess, tree.getChild(i).getChild(0).getText(), tds, f);
                            if (typeMess != null) {
                                break;
                            }
                            i++;
            ...
            case bloodResetLexer.WHILE:
                TDS tdsWhile = new TDS(tds, "WHILE", "while"+TDS.getCountWhile());
